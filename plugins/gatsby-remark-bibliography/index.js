...
module.exports = ({ markdownAST, markdownNode, getNode }, { components }) => {
  ...
  if (fs.existsSync(bibtex_f)) {
    ...
    const parsedEntries = bibtexParse.parse(bibtex);
    for (const entry of parsedEntries) {
      let key = entry.key
      let fields = {}
      fields['type'] = entry.type
      if (entry.itemtype == 'entry'){
        for (const [_, field] of Object.entries(entry.fields)) {
          fields[field.name.toLowerCase()] = normalizeTag(field.value);
        }
        bibliography.set(key, fields);
      }
    }
  }
  ...
  visit(markdownAST, "html", (node, index, parent) => {
    if (bibliography.size > 0 && node.value.startsWith(`<bibliography>`)) {
      let res = '<ol>';
